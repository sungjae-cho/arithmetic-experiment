...
file_parameters = 'parameters.pickle'



def import_data(operator, input_bits):
    '''
    operator: 'add', 'subtract', 'multiply', 'divide' 
    '''
    dir_data_request = '{}/{}-bit/{}'.format(dir_data, input_bits, operator)
    
    with open('{}/input_train.pickle'.format(dir_data_request), 'rb') as f:
        input_train = pickle.load(f)
    with open('{}/input_dev.pickle'.format(dir_data_request), 'rb') as f:
        input_dev = pickle.load(f)
    with open('{}/input_test.pickle'.format(dir_data_request), 'rb') as f:
        input_test = pickle.load(f)

    with open('{}/output_train.pickle'.format(dir_data_request), 'rb') as f:
        output_train = pickle.load(f)
    with open('{}/output_dev.pickle'.format(dir_data_request), 'rb') as f:
        output_dev = pickle.load(f)
    with open('{}/output_test.pickle'.format(dir_data_request), 'rb') as f:
        output_test = pickle.load(f)
    
    return (input_train, input_dev, input_test, 
           output_train, output_dev, output_test)

def import_parameters(run_id, operation):
    '''
    run_id: string of run ID
    operation: 'add', 'subtract', 'multiply', 'divide' 
    Return: parameters
    - parameters['h1/kernel']
    - parameters['h1/bias']
    - parameters['last_logits/kernel']
    - parameters['last_logits/bias']
    '''
    with open('{}/{}/parameters/{}/{}'.format(dir_data, operation, run_id, file_parameters).format(), 'rb') as f:
...
def write_run_info(run_info,
                   float_epoch, 
                   dev_loss_val, dev_accuracy_val, dev_op_wrong_val,
                   dev_loss_tlu_val, dev_accuracy_tlu_val, dev_op_wrong_tlu_val,
    ...
    for i in range(len(per_digit_wrong_val)):
        if per_digit_wrong_val[-(i+1)] == 0 and run_info['init_all_correct_digit-{}_epoch'.format(i+1)] == -1:
            run_info['init_all_correct_digit-{}_epoch'.format(i+1)] = float_epoch
        if per_digit_wrong_val[-(i+1)] == 0 and run_info['init_complete_all_correct_digit-{}_epoch'.format(i+1)] == -1:
            run_info['init_complete_all_correct_digit-{}_epoch'.format(i+1)] = float_epoch
        if per_digit_wrong_val[-(i+1)] != 0 and run_info['init_complete_all_correct_digit-{}_epoch'.format(i+1)] != -1:
            run_info['init_complete_all_correct_digit-{}_epoch'.format(i+1)] = -1
    
    # Save run_info
    create_dir(run_info_dir)
    with open('{}/run-{}.pickle'.format(run_info_dir, run_id), 'wb') as f:
        pickle.dump(run_info, f)
        
def write_measures(run_id,
                   float_epoch, 
                   dev_loss_val, dev_accuracy_val, dev_op_wrong_val, 
                   dev_loss_tlu_val, dev_accuracy_tlu_val, dev_op_wrong_tlu_val,
                   per_digit_accuracy_val, per_digit_wrong_val):
    
    create_dir(dir_measure_log)
    
    if not os.path.exists('{}/run-{}.pickle'.format(dir_measure_log, run_id)):
        ...
        with open('{}/run-{}.pickle'.format(dir_measure_log, run_id), 'rb') as f:
    ...
    with open('{}/run-{}.pickle'.format(dir_measure_log, run_id), 'wb') as f:
        pickle.dump(measure_logs, f)
    
        
def read_measure_logs(run_id):
    with open('{}/run-{}.pickle'.format(dir_measure_log, run_id), 'rb') as f:
        measure_logs = pickle.load(f)
    return measure_logs
